...
function doDownload1() {
  ...
  var resultArr = splitArr(contentText);
  ...
  for (var j = 0; j < resultArr.length; j++) {
    ...
    downloadLink.download = dateFormat(new Date(), "yyyy-MM-dd") + '生成的序列号从' + $('.serialNumberLeft').val() + '到' + $('.serialNumberRight').val()+'之间的sql语句' +'(' + j + ')' + ".txt"; // 下载的文件名称
